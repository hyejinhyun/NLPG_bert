...
def output_write(output_predict_file, predicted_token, logit_probability, answer):
    ...
    with open(output_predict_file, 'at', -1, "utf-8") as out_file:
        ...
        tsv_writer.writerow([predicted_token, logit_probability, answer])

def convert_data_to_context(filepath):
    dataset = pd.read_csv(filepath, delimiter = '\t', encoding = 'CP949')
...
if __name__ == "__main__":
    ...
    question_inputs = convert_data_to_context('MLM2.tsv')
    question_answers = get_answer('MLM2.tsv')
    ...
    for question_input in question_inputs:
        ...
        output_write("blank_test_results2.tsv", predicted_token, logit_prob[predicted_index], question_answers[index])